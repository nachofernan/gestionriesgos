...
<div class="max-w-2xl space-y-6">
    ...
    <div class="flex items-center gap-3">
        <a href="{{ route('auditoria.objetivos.show', $objetivo) }}" class="text-gray-400 hover:text-gray-600 transition-colors">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                <path fill-rule="evenodd" d="M9.707 14.707a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414l4-4a1 1 0 011.414 1.414L7.414 9H15a1 1 0 110 2H7.414l2.293 2.293a1 1 0 010 1.414z" clip-rule="evenodd" />
            </svg>
        </a>
        <h1 class="text-2xl font-extrabold text-gray-900">Editar Objetivo</h1>
    </div>

    <form action="{{ route('auditoria.objetivos.update', $objetivo) }}" method="POST">
        @csrf @method('PATCH')
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 space-y-5">

            <div>
                <label class="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">Nombre *</label>
                <input type="text" name="nombre" value="{{ old('nombre', $objetivo->nombre) }}"
            ...
            <div>
                ...
                <textarea name="descripcion" rows="3"
                    class="w-full border-gray-200 rounded-xl px-4 py-3 text-sm focus:border-indigo-500 focus:ring-indigo-500">{{ old('descripcion', $objetivo->descripcion) }}</textarea>
            </div>

            <div x-data="{ noAplica: {{ old('fecha_objetivo', $objetivo->fecha_objetivo) ? 'false' : 'true' }} }">
                <label class="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">Fecha Objetivo</label>
                <div class="flex items-center gap-3 mb-2">
                    <input type="hidden" name="fecha_objetivo" value="">
                    <input type="date" name="fecha_objetivo" value="{{ old('fecha_objetivo', $objetivo->fecha_objetivo?->format('Y-m-d')) }}"
                        :disabled="noAplica"
                ...
                <div>
                    ...
                    <select name="area_id"
                        ...
                        @foreach ($areas as $area)
                            <option value="{{ $area->id }}" {{ old('area_id', $objetivo->area_id) == $area->id ? 'selected' : '' }}>
                                {{ $area->nombre }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select name="user_id"
                        ...
                        @foreach ($usuarios as $usuario)
                            <option value="{{ $usuario->id }}" {{ old('user_id', $objetivo->user_id) == $usuario->id ? 'selected' : '' }}>
                                {{ $usuario->name }}
                            </option>
                        @endforeach
                    </select>
