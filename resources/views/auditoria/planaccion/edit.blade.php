    ...
    <form action="{{ route('auditoria.planes.update', $planAccion) }}" method="POST">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 space-y-5">
            ...
            <div>
                ...
                <input type="text" name="codigo" value="{{ old('codigo', $planAccion->codigo) }}"
            ...
            <div>
                ...
                @php $riesgoIdsActuales = $planAccion->riesgos->pluck('id')->toArray(); @endphp
                <div class="space-y-2 max-h-52 overflow-y-auto border border-gray-200 rounded-xl p-3">
                    @forelse ($riesgos as $riesgo)
                        <label class="flex items-center gap-3 p-2.5 rounded-lg hover:bg-orange-50/30 cursor-pointer transition-all
                            {{ in_array($riesgo->id, $riesgoIdsActuales) ? 'bg-orange-50/50 border border-orange-100' : '' }}">
                            <input type="checkbox" name="riesgo_ids[]" value="{{ $riesgo->id }}"
                                {{ in_array($riesgo->id, old('riesgo_ids', $riesgoIdsActuales)) ? 'checked' : '' }}
                    ...
                    @empty
                        <p class="text-sm text-gray-400 italic p-2">No hay riesgos disponibles.</p>
                    @endforelse
                </div>
            </div>
                ...
                <div>
                    ...
                    <select name="area_id"
                        ...
                        @foreach ($areas as $area)
                            <option value="{{ $area->id }}" {{ old('area_id', $planAccion->area_id) == $area->id ? 'selected' : '' }}>
                                {{ $area->nombre }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select name="user_id"
                        ...
                        @foreach ($usuarios as $usuario)
                            <option value="{{ $usuario->id }}" {{ old('user_id', $planAccion->user_id) == $usuario->id ? 'selected' : '' }}>
                                {{ $usuario->name }}
                            </option>
                        @endforeach
                    </select>
